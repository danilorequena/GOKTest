...
final class ProductsSectionSpec: QuickSpec {
    override func spec() {
        describe("ProductsViewSection") {
            var collectionView: UICollectionView!
            var sut: ProductsSection!
            var delegateMock: ProductsSectionDelegateMock!
            
            beforeEach {
                delegateMock = .init()
                collectionView = UICollectionView(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout())
                sut = .init(products: Product.mockArray())
            ...
            describe("collectionView(_:didSelectItemAt:)") {
                ...
                it("has to invoke delegate to select banner") {
                    expect(delegateMock.hasCalledDidTapProduct) == true
                }
            }
            ...
            describe("collectionView(_:cellForItemAt:)") {
                ...
                context("when banner is of type 1") {
                    it("has to have a correct type") {
                        ...
                        expect(cell).to(beAnInstanceOf(ProductsCollectionViewCell.self))
                    }
                }
